...
import com.tokio.pa.cargoquotation73.bean.LogVoBo;
import com.tokio.pa.cargoquotation73.bean.ModoAuxiliar;
...
@Component(
		 ...
		 "mvc.command.name=/CotizadorTransportistas/Paso3Next"
		 },
		 service = MVCActionCommand.class
		 )

public class SavePaso3ActionCommand extends BaseMVCActionCommand {
	
	private static final Log _log = LogFactoryUtil.getLog(SavePaso3ActionCommand.class);
	...
	protected void doProcessAction(ActionRequest actionRequest, ActionResponse actionResponse) throws Exception {
		...
		actionResponse.getRenderParameters().setValue("jspPage", "/pantallas/paso4.jsp");
	}
	...
	private void fQuienEsQuien(ActionRequest actionRequest){
		...
		try {
			...
			WebResource webResource = client.resource("https://qeq.com.mx/datos/qws/access")
					.queryParam("var1", CargoQuotation73PortletKeys.QQ_USUARIO)
					.queryParam("var2", CargoQuotation73PortletKeys.QQ_CONTRASENA);
			...
			if (Validator.isNull(cookieId)) {
				...
				if (isMoral){
					...
					WebResource	razMor = client.resource(CargoQuotation73PortletKeys.QQ_URL_MORAL)
							.queryParam("razonsoc",
									HtmlUtil.escape( responseEmision.getDatosCliente().getNombre().toLowerCase()));					
					
					WebResource	rfcMor = client.resource(CargoQuotation73PortletKeys.QQ_URL_MORAL)
							.queryParam("rfc", responseEmision.getDatosCliente().getRfc());

					WebResource	nom_den_rfc = client.resource(CargoQuotation73PortletKeys.QQ_URL_MORAL)
							.queryParam("razonsoc",
									HtmlUtil.escape( 
					...
					WebResource nomCom = client.resource(CargoQuotation73PortletKeys.QQ_URL_FISICA)
					...
					WebResource nomRFC = client.resource(CargoQuotation73PortletKeys.QQ_URL_FISICA)
							.queryParam("nombre", HtmlUtil.escape(responseEmision.getDatosCliente().getNombre().toLowerCase()))
							.queryParam("rfc", responseEmision.getDatosCliente().getRfc());					

					WebResource apPRFC = client.resource(CargoQuotation73PortletKeys.QQ_URL_FISICA)
							.queryParam("paterno", HtmlUtil.escape(responseEmision.getDatosCliente().getAppPaterno().toLowerCase()))
							.queryParam("rfc", responseEmision.getDatosCliente().getRfc());					

					WebResource apMRFC = client.resource(CargoQuotation73PortletKeys.QQ_URL_FISICA)
							.queryParam("materno", HtmlUtil.escape(responseEmision.getDatosCliente().getAppMaterno().toLowerCase()))
							.queryParam("rfc", responseEmision.getDatosCliente().getRfc());
